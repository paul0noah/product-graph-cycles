#ifndef DIJK_BNB_CUDA_hpp
#define DIJK_BNB_CUDA_hpp
...
template<typename DTYPE>
__global__ void printCudaMatrix(const CudaMatrix<DTYPE> mat, int rows = -1, int cols = -1) {
    if (rows == -1) rows = mat.rows();
    if (cols == -1) cols = mat.cols();
...
typedef struct LBTUPLE {
    ...
    __device__ __host__ LBTUPLE(float _lb, int _v) {
        lb = _lb;
        v = _v;
    }
...
struct DKSTRAGPUSTATE {
    ...
    CudaMatrix<PRECISION_DIJK> costCuda;
    CudaMatrix<int> predecessor;
    CudaMatrix<PRECISION_DIJK> DPtable;
    thrust::host_vector<LBTUPLE> lowerBounds;
    ...
    PRECISION_DIJK ratioLowerBoundOfCurrentBranch;
    PRECISION_DIJK ratioLowerBound;
    PRECISION_DIJK ratioUpperBound;
    ...
    PRECISION_DIJK currentRatio;
    PRECISION_DIJK tolerance;
    Eigen::MatrixX<PRECISION_DIJK> branchTable;
    CYCLE cycleCuda;
    thrust::host_vector<int> cycle;
